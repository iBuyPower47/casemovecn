...
const inventoryReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case 'INVENTORY_STORAGES_ADD_TO':
      console.log(state);
      const add_to_filtered =
        state.storageInventory?.filter(
          (id) => id.storage_id != action.payload.casketID
        ) || [];
      const add_to_filtered_raw =
        state.storageInventoryRaw?.filter(
          (id) => id.storage_id != action.payload.casketID
        ) || [];
      action.payload.storageData.forEach((storageRow) =>
        add_to_filtered.push(storageRow)
      );
      action.payload.storageRowsRaw.forEach((storageRow) =>
    ...
    case 'INVENTORY_STORAGES_CLEAR_CASKET':
      const AddToFiltered = state.storageInventory.filter(
        (id) => id.storage_id != action.payload.casketID
      );
      const AddToFilteredRaw = state.storageInventoryRaw.filter(
        (id) => id.storage_id != action.payload.casketID
      );
